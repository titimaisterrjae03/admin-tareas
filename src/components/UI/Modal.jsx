...
export function Modal({ isOpen, onClose, title, children }) {
    ...
    return createPortal(
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm p-0 md:p-4 animate-in fade-in duration-200">
            <div className="bg-slate-900 border-t md:border border-slate-700 w-full max-w-md rounded-t-2xl md:rounded-xl shadow-2xl animate-in slide-in-from-bottom-10 duration-300 max-h-[90vh] overflow-y-auto flex flex-col">
                ...
                <div className="p-4 safe-area-bottom overflow-y-auto">
                    {children}
                </div>
            </div>
        </div>,
